...
public class Figura {
    ...
    public boolean adelante, atras, izquierda, derecha, caminando, corriendo, quieto, disparo, reinicio, salto,accion;
    ...
    public int[] estadoEntornoFigura;      //El entorno alrededor de la figura podria descrbirse con mas de un estado. Lloviendo y tengo poca energia
    public Figura objetivo;                      //El objetivo puede ser: localizar otra figura,
    //Si ademas, hubiera que realizar una accion particular (ej. Dispararle, darle alimento) se necesitaria otro atributo (ej. TareaObjetivo)
    ...
    public void crearPropiedades(float masa, float elasticidad, float dumpingLineal, float posX, float posY, float posZ, DiscreteDynamicsWorld mundoFisico) {
        ...
        ramaVisible.setUserData("figra_" + listaObjetosFisicos.size());
    ...
    public void mostrar(RigidBody cuerpoRigido) {
        ...
        if (cuerpoRigido != null && cuerpoRigido.getMotionState() != null) {
            ...
            this.posiciones[0] = (float) (trans.origin.x);
    ...
    public void actualizarLocalizacion(float x, float y, float z) {
        
    }

    //permite obtener un vector para empujar al personaje
    public Vector3f conseguirDireccionFrontal() {
        ...
        Vector3f posSonar = new Vector3f(0, 0, 0);
        copiat3dPersonaje.get(posSonar);
        Vector3f posPersonaje = new Vector3f(0, 0, 0);
        t3dPersonaje.get(posPersonaje);
        return new Vector3f((posSonar.x - posPersonaje.x) * 10, -(posSonar.y - posPersonaje.y), (posSonar.z - posPersonaje.z) * 10);
    }

    //permite obtener un vector para empujar hacia atras al personaje
    public Vector3f conseguirDireccionTrasera() {
        Transform3D t3dPersonaje = new Transform3D();
        this.desplazamientoFigura.getTransform(t3dPersonaje);
        Transform3D copiat3dPersonaje = new Transform3D(t3dPersonaje);
        Transform3D t3dSonar = new Transform3D();
    ...
    public double DistanciaColisionEntreFiguras(Figura F2) {   //
